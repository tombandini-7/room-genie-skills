...
3. Collect: dates, adults, children + each child's age, ticketDays (how many days in park?), ticketType, diningPlan, memoryMaker?, travelProtection?
4. `explore_rates({ mode: "package", ... })` — this can take 2–4 minutes; warn the user.
5. Render the resulting table. Highlight: nightly rate, package grand total, deposit ($200 + travel protection), balance due.
